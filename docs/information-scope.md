...
PMH は今回の監視範囲には入れない。自治体個別ページは重要な隣接領域であり、実制度文脈を見るために Source Health の取得状態確認として扱う。自治体制度全体の網羅ではなく、支払基金・厚労省側の地単公費マスター更新を中心に追う。
...
| 補助 | 支払基金トップ更新情報 | 更新情報に出るかの補助確認 | Source Health で取得状態を見る。更新有無の本命判断には使わない |
| seed | 自治体個別の医療費助成制度ページ | 受給者証、自己負担、現物給付、償還払いなどの実制度文脈 | Source Health で取得状態を見る。通常 RSS には出さない |
...
| 支払基金トップ更新情報 | titansys 更新がトップに出るか | Source Health で取得状態を見る。補助確認に留める |

RSS は、公式ページや公開ファイルの変化を知らせる入口である。制度変更の確定判断、本番マスター反映、ベンダー作業指示を自動で意味しない。Severity は確認優先度、Review は人の確認要否、確度は根拠の強さ、Source Health 用の項目は取得状態の補助確認として扱う。自治体 seed は Source Health で取得状態を見る。
...
| `municipality-policy-seed` | 自治体制度ページを実制度文脈として見る Source Health 用 seed | 札幌市、横浜市、墨田区など |

## 対象外を見るときの注意

地単公費マスターに載っていないからといって、自治体制度が存在しないとは限らない。自治体 seed は、この読み違いを避けるための補助資料である。特に次を分けて見る。
...
自治体 seed は、支払基金マスターの更新を検知する本命ソースではない。自治体の制度ページには、対象者、受給者証、自己負担、所得制限、現物給付、償還払いなど、マスターだけでは読めない実制度の言葉が載る。

Chitan Watch では、札幌市、横浜市、墨田区のような自治体ページを初期 seed として Source Health に出す。通常 RSS には出さない。将来、支払基金マスター差分と自治体ページの記述を突合できる段階になったら、補助根拠として扱う。
