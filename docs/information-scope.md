...
PMH は今回の監視範囲には入れない。自治体個別ページは、支払基金マスターとは別の「自治体制度文脈」として現在の監視範囲に含める。自治体制度全体の網羅ではなく、支払基金・厚労省側の地単公費マスター更新と、代表的な自治体制度ページの文脈更新を分けて追う。
...
| 4 | 自治体個別の医療費助成制度ページ | 受給者証、自己負担、現物給付、償還払いなどの実制度文脈 | 自治体制度文脈更新として RSS に出す |
...
RSS は、公式ページや公開ファイルの変化を知らせる入口である。制度変更の確定判断、本番マスター反映、ベンダー作業指示を自動で意味しない。Severity は確認優先度、Review は人の確認要否、確度は根拠の強さとして扱う。自治体制度文脈は、支払基金マスター更新とは別の見出しで読む。
...
| `municipality-policy-context` | 自治体制度ページを実制度文脈として見る現在スコープのレイヤー | 札幌市、横浜市、墨田区など |

## 対象外を見るときの注意

地単公費マスターに載っていないからといって、自治体制度が存在しないとは限らない。自治体制度文脈の監視は、この読み違いを避けるために現在スコープへ含める。特に次を分けて見る。
...
自治体制度ページは、支払基金マスターの実ファイルを置く場所ではない。自治体の制度ページには、対象者、受給者証、自己負担、所得制限、現物給付、償還払い、申請、更新日など、マスターだけでは読めない実制度の言葉が載る。

Chitan Watch では、札幌市、横浜市、墨田区のような自治体ページを現在スコープの制度文脈として扱う。RSS では「自治体制度文脈更新」として出し、支払基金マスターの最新データ更新とは見出しを分ける。支払基金マスター差分と自治体ページの記述を突合する機能も、この現在スコープの延長として育てる。
...
flowchart LR
  ...
  J --> L[Web]
  I --> M[Source Health]
  N[自治体制度ページ] --> O[制度文脈 Snapshot]
  O --> P[自治体制度文脈 ChangeEvent]
  P --> Q[RSS 別見出し]
  P --> R[Web]
```
