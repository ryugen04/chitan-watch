...
class StaticExportTest(unittest.TestCase):
    ...
    def test_export_static_site_writes_web_json_and_rss(self):
        with tempfile.TemporaryDirectory() as store_dir, tempfile.TemporaryDirectory() as output_dir:
            ...
            self.assertIn("static/master-versions.json", result.files)
            self.assertIn("static/master-diffs.json", result.files)
            self.assertIn("static/llm-status.json", result.files)
            self.assertIn("static/llm-interpretations.json", result.files)
            self.assertTrue(any(name.startswith("static/master-diffs/") and name.endswith(".json") for name in result.files))
            ...
            master_versions = json.loads((output / "static/master-versions.json").read_text(encoding="utf-8"))
            master_diffs = json.loads((output / "static/master-diffs.json").read_text(encoding="utf-8"))
            llm_status = json.loads((output / "static/llm-status.json").read_text(encoding="utf-8"))
            llm_interpretations = json.loads((output / "static/llm-interpretations.json").read_text(encoding="utf-8"))
            detail_path = output / master_diffs["diffs"][0]["detail_url"]
            ...
            self.assertEqual(2, master_versions["version_count"])
            self.assertEqual(1, master_diffs["diff_count"])
            self.assertEqual(master_diffs["diffs"][0]["diff_id"], detail["diff_id"])
            self.assertEqual("disabled", llm_status["status"])
            self.assertEqual([], llm_interpretations["interpretations"])
            self.assertNotIn(str(Path(store_dir)), json.dumps(master_versions, ensure_ascii=False))
            self.assertNotIn("payloads/", json.dumps(master_diffs, ensure_ascii=False))
    ...
    def test_web_app_references_static_json_fallback(self):
        ...
        self.assertIn("static/master-versions.json", app_js)
        self.assertIn("static/master-diffs.json", app_js)
        self.assertIn("static/llm-status.json", app_js)
        self.assertIn("#master-compare", app_js)
