...
class ProductSliceTest(unittest.TestCase):
    ...
    def test_registry_backed_run_emits_master_and_document_events(self):
        ...
        self.assertIn("master-publication", health["source_groups"])
        self.assertIn("pmh-online-qualification", health["source_layers"])
        self.assertIn("municipality-policy", health["source_layers"])
        self.assertTrue(any(source.get("source_owner") == "digital-agency" for source in health["sources"]))
        self.assertTrue(any(source.get("notify_policy") == "always" for source in health["sources"]))
        self.assertFalse(any("source-layer:municipality-policy" in event.get("change_categories", []) for event in events))
    ...
    def test_web_app_contains_api_fallback_and_product_routes(self):
        ...
        self.assertIn('pmh-online-qualification', app_js)
        self.assertIn('municipality-policy', app_js)
